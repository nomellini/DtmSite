...
class ViewTreinamentoPage extends React.Component {
  ...
  confirmarInscricao(e) {
    ...
    if (aceitepagamento) {
      this.setState({ msgaceitepagamento: '' });

      if (idTreinamento && CPF && modulos) {
      ...
      this.setState({ msgaceitepagamento: 'O termo de pagamento deve ser aceito' });
  ...
  handleSubmitInscricao(e) {
    ...
    if (CPF) {
      ...
      grupoService.UserExists(CPF).then(response =>
        response.text().then(text => {
          ...
          if (!response.ok) {
            ...
            this.setState({ submitted: false });
            this.setState({ modeLogin: false, submitted: false, step: '2' });
  ...
  render() {
    const { user, users, grupos, anothersgrupos, estados, cidades, cidadesmodais, loading, sucess } = this.props;
    const { estadoSelectedModal, cidadeselectedModal, nome, codigo, empresa, submitted, turmaSelected, CPF, msgerroLogin, modeLogin, step, telefone, cargo, email, aceitepagamento ,msgaceitepagamento} = this.state;
                          ...
                          <div className="modal-content">
                            ...
                            <div className="modal-body">
                              <form role="form" onSubmit={this.handleSubmit}>
                                ...
                                <div className="msg-erro" style={{color: 'red', textAlign: 'center'}}>
                                  {submitted && msgaceitepagamento && <div>

                                    <img src="../../public/images/icon-alert.svg" />
                                    <span className="error">{msgaceitepagamento}</span></div>}
                                </div>
                              </form>
                            </div>
